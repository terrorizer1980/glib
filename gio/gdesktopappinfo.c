  ...
  do
    {
      ...
      const gchar *const wrapper_argv[] = {
        ...
        "-e",
        "-u",
        /* The "cat" will hang until the write side is closed, "$signal_fd<&-" closes the read side */
        "-c", "export GIO_LAUNCHED_DESKTOP_FILE_PID=$$; signal_fd=$1; shift; cat <&$signal_fd; exec \"$@\" $signal_fd<&-",
        "sh", /* argv[0] for sh */
